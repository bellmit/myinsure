...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            globalTabIdIndex++;
            id = data.href.split('/')[data.href.split('/').length-1];
            var content = '<iframe id="'+ id +'" src="' + data.href + '" data-id="' + globalTabIdIndex + '"></iframe>';
            var title = '';
...
//                _config.elem.find('div.layui-tab-content > div').eq(tabIndex).children('iframe')[0].contentWindow.location.reload();
        ...
        if (_config.onSwitch) {
            ...
            //单击选项卡时，进行重新加载数据
            /*ELEM.titleBox.find('li').on('click', function (e) {
            	var $target = e.target.nodeName === 'LI' ? e.target : e.target.parentElement;
        		//获取当前点击选项卡的id值
                var id = $($target).find('i.layui-tab-close').data('id');
                //为iframe重新赋值src
