...
layui.define(['element', 'common'], function (exports) {
    ...
    Navbar.prototype.render = function () {
        ...
        if (_config.spreadOne) {
            var $ul = $container;
//            $ul.find('.foo').each(function () {
//                $(this).on('click', function () {
//                    $(this).siblings().removeClass('open');
//                });
//            });
            $ul.find('.foo').eq(0).click();
        }
        return _that;
    };
	/**
	...
	/**
	 * 获取html字符串,这是我自己修改的内容
	 * @param {Object} data
	 */
    function getHtml(data) {
        //debugger;
    	var ulHtml = '';
        for (var i = 0; i < data.length; i++) {//循环父级菜单
        	//如果当前父级菜单有子菜单
            if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
            	var fooHtml  = '';
            	if(i == 0){
            		fooHtml  = '<li class="foo open">';
            	}else{
            		fooHtml  = '<li class="foo">';
            	}
            	    fooHtml = $(fooHtml + '<a href="#" class="menu-dropdown">'
            	...
            	var sonHtml = $('<ul class="submenu"></ul>');
            	for (var j = 0; j < data[i].children.length; j++) {
            		var son = '<li class="active">'
                            +     '<a href="../' + data[i].children[j].href + '" target="content">'
                            +          '<span class="menu-text">' + data[i].children[j].title + '</span>'
                            +     '</a>'
                            +  '</li>';
            		sonHtml.append(son);
                }
            	fooHtml.append(sonHtml);
            	var div = $('<div></div>').append(fooHtml);
            	ulHtml += div.html();
            } else {
            }
        }
